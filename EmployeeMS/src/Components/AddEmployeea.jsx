...
const AddEmployeea = () => {
    const [employeee, setEmployeee] = useState({
     ...
     useEffect( () => {
       ...
       const handleSubmit = (e) => {
         ...
         formData.append('address',employeee.address);
         formData.append('salary',employeee.salary);
         formData.append('sex',employeee.sex);
         formData.append('image',employeee.image);
         formData.append('category_id',employeee.category_id);
         axios.post('http://localhost:3000/auth/add_employeea', formData)
         .then(result =>{
           if(result.data.Status){
  ...
  return (
    ...
    <Link to="/sidebar/employee" className='btn btn-primary mt-4 ml-4 w-30'>Back</Link>
     <h2 className='text-3xl font-bold text-center'>Add Employee</h2>
      <form className=' mx-0 row g-1 ' onSubmit={handleSubmit} >
        ...
        <div className='col-12 '>
         <label for="inputAddress" className='form-label text-xl'>Address</label>
         
         <input type="text"  
        className='form-control rounded-0' id='inputAddress'
          placeholder='1234 Robe' autoComplete='off' onChange={(e) => setEmployeee({...employeee, address: e.target.value})} 
        />
        ...
        <div className='col-12 '>
         ...
         onChange={(e) => setEmployeee({...employeee, category_id: e.target.value})} >
         {category.map((c) => {
